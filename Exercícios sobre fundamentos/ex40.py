#Exercício Python 040: Crie um programa que leia duas notas de um aluno e
# calcule sua média, mostrando uma mensagem no final, de acordo com a média atingida:

#– Média abaixo de 5.0: REPROVADO

#– Média entre 5.0 e 6.9: RECUPERAÇÃO

#– Média 7.0 ou superior: APROVADO

n1 = float(input('Digite a primeira nota '))
n2 = float(input('Digite a segunda nota '))
med = (n1+n2)/2
if(med > 7.0):
    print('A sua média é {} logo você foi aprovado '.format(med))
elif(med > 5.0 and med < 6.9):
    print('Sua média foi {} logo você está de recuperação'.format(med))
else:
    print('Sua média foi {} então você foi reprovado'.format(med))